...
def setup_enhanced_cloze_menu() -> None:
    def on_main_window_did_init():
        ...
        submenu = menu.addMenu("Cloze TTS-Input")
...
def add_reset_notetype_action_to_menu(menu: QMenu) -> None:
    action = menu.addAction("Reset 'Cloze TTS-Input' note type")

    def on_triggered():
        if not askUser(
            "This will reset the 'Cloze TTS-Input' note type to its default version.\n\n"
        ...
        tooltip("Successfully reset 'Cloze TTS-Input' note type.")

    qconnect(action.triggered, on_triggered)


def add_reset_css_action_to_menu(menu: QMenu) -> None:
    action = menu.addAction("Reset 'Cloze TTS-Input' note type styling (css)")

    def on_triggered() -> None:
        if not askUser(
            "This will reset the styling (css) of the 'Cloze TTS-Input' note type to its default version.\n\nContinue?"
        ):
            return
        ...
        tooltip("Successfully reset 'Cloze TTS-Input' note type styling.")
