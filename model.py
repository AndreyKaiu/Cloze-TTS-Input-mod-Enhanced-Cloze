...
def version(note_type: "NotetypeDict") -> Optional[Tuple[int, ...]]:
    ...
    m = re.match("<!-- VERSION (.+?) mod kaiu -->", front)
    if not m:
        return None
...
def set_version(front: str, version: Tuple[int, ...]) -> str:
    return re.sub(
        "<!-- VERSION (.+?) mod kaiu -->",
        f"<!-- VERSION {'.'.join(map(str, version))} mod kaiu -->",
        front,
    )
...
def load_enhanced_cloze(note_type: "NotetypeDict") -> None:
    front_path = NOTE_TYPE_DIR / "Cloze_TTS_Input_Front_Side.html"
    css_path = NOTE_TYPE_DIR / "Cloze_TTS_Input_CSS.css"
    back_path = NOTE_TYPE_DIR / "Cloze_TTS_Input_Back_Side.html"
